...
namespace UnityDebugViewer
{
    ...
    public class DebugViewer : MonoBehaviour
    {
        ...
        /// 用于保存采样数据
        /// </summary>
        List<DebugSampleData> _sampleDataList = new List<DebugSampleData>();

        /// <summary>
        /// 保存uncollapsed log数据
        /// </summary>
        List<DebugLogData> _logList = new List<DebugLogData>();
        /// <summary>
        /// 保存collapsed log数据
        /// </summary>
        List<DebugLogData> _collapsedLogList = new List<DebugLogData>();
        /// <summary>
        /// 保存当前展示的log数据
        /// </summary>
        List<DebugLogData> _currentLogList = new List<DebugLogData>();

        /// <summary>
        /// 用来判断当前加入的log是否已存在
        /// </summary>
        MultiKeyDictionary<string, string, DebugLogData> _logsDic = new MultiKeyDictionary<string, string, DebugLogData>();
        //to save memory
        ...
        void OnEnable()
        {
            if (_logList.Count == 0)//if recompile while in play mode
                ClearData();
        }
        ...
        void AddSample()
        {
            DebugSampleData sample = new DebugSampleData();
            sample.fps = fps;
            sample.fpsText = fpsText;
            sample.time = Time.realtimeSinceStartup;
            sample.memory = gcTotalMemory;
            _sampleDataList.Add(sample);

            graphMemUsage = (_sampleDataList.Count * DebugSampleData.MemorySize()) / 1024 / 1024;
        }
        ...
        void CalculateCurrentLog()
        {
            ...
            if (collapse)
            {
                for (int i = 0; i < _collapsedLogList.Count; i++)
                {
                    DebugLogData log = _collapsedLogList[i];
            ...
            if (selectedLog != null)
            {
                int newSelectedIndex = _currentLogList.IndexOf(selectedLog);
                if (newSelectedIndex == -1)
                {
                    DebugLogData collapsedSelected = _logsDic[selectedLog.condition][selectedLog.stacktrace];
                    newSelectedIndex = _currentLogList.IndexOf(collapsedSelected);
                    if (newSelectedIndex != -1)
        ...
        void drawToolBar()
        {
            ...
            if (showCopyAllButton)
            {
                if (GUILayout.Button(copyAllContent, barStyle, GUILayout.Width(size.x * 2), GUILayout.Height(size.y * 2)))
                {
                    ...
                    _logList.ForEach(l => allLogsToClipboard += l.condition + System.Environment.NewLine + System.Environment.NewLine + l.stacktrace);

                    if (string.IsNullOrEmpty(allLogsToClipboard))
                        GUIUtility.systemCopyBuffer = "No log selected";
                    else
        ...
        void DrawLogs()
        {
            ...
            int totalCount = _currentLogList.Count;
            ...
            for (int i = startIndex; (startIndex + index) < endIndex; i++)
            {
                ...
                if (i >= _currentLogList.Count)
                    break;
                DebugLogData log = _currentLogList[i];
                ...
                DebugSampleData sample = _sampleDataList[log.sampleId];
                fpsRect = countRect;
                if (showFps)
                {
        ...
        void drawGraph()
        {
            ...
            if (graphScrollerPos.x >= (_sampleDataList.Count * graphSize - Screen.width))
                graphScrollerPos.x += graphSize;

            GUILayout.Label(" ", GUILayout.Width(_sampleDataList.Count * graphSize));
            GUILayout.EndScrollView();
            GUILayout.EndArea();
            maxFpsValue = 0;
            minFpsValue = 100000;
            ...
            for (int i = 0; i < Screen.width / graphSize; i++)
            {
                ...
                if (index >= _sampleDataList.Count)
                    break;
                DebugSampleData s = _sampleDataList[index];
            ...
            if (currentFrame != -1 && currentFrame < _sampleDataList.Count)
            {
                DebugSampleData selectedSample = _sampleDataList[currentFrame];
        ...
        void drawStack()
        {
            ...
            if (selectedLog != null)
            {
                ...
                DebugSampleData selectedSample = null;
                try
                {
                    selectedSample = _sampleDataList[selectedLog.sampleId];
                }
        ...
        void calculateStartIndex()
        {
            ...
            startIndex = Mathf.Clamp(startIndex, 0, _currentLogList.Count);
        }
	...
	float lastUpdate2 = 0;
        ...
        void Update()
        {
            ...
            if (threadedLogs.Count > 0)
            {
                lock (threadedLogs)
                {
                    ...
                    {
                        DebugLogData l = threadedLogs[i];
                        AddLog(l.condition, l.stacktrace, (UnityEngine.LogType)l.logType);
                    }
                    threadedLogs.Clear();
                }
            }
        ...
        void CaptureLog(string condition, string stacktrace, UnityEngine.LogType type)
        {
            AddLog(condition, stacktrace, type);
        }

        void AddLog(string condition, string stacktrace, UnityEngine.LogType type)
        {
            float memUsage = 0f;
            string _condition = "";
            ...
            AddSample();
            DebugLogData log = new DebugLogData() { logType = (LogType)type, condition = _condition, stacktrace = _stacktrace, sampleId = _sampleDataList.Count - 1 };
            memUsage += log.MemorySize();
        ...
        List<DebugLogData> threadedLogs = new List<DebugLogData>();
        void CaptureLogThread(string condition, string stacktrace, UnityEngine.LogType type)
        {
            DebugLogData log = new DebugLogData() { condition = condition, stacktrace = stacktrace, logType = (LogType)type };
            lock (threadedLogs)
            {
        ...
        private void SaveLogsToDevice()
        {
            ...
            for (int i = 0; i < _logList.Count; i++)
            {
                fileContentsList.Add(_logList[i].logType + "\n" + _logList[i].condition + "\n" + _logList[i].stacktrace);
            }
